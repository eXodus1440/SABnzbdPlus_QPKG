...
#PYTHON_PATH=`$CMD_GETCFG python Install_Path -f ${CONF}`/bin/python
...
if [ -z $BASE ] ; then
  ...
  $CMD_WLOG "[SABnzbdPlus] The Public share not found." 1
  exit 1
fi
...
case "$1" in
  start)
    ...
    if [ -f ${QPKG_DIR}/sabnzbd-${WEBUI_PORT}.pid ]; then
      echo "SABnzbdPlus is currently running or hasn't been shutdown properly. Please stop it before starting a new instance."
      #echo "If not so, then delete the file named ${QPKG_DIR}/sabnzbd-${WEBUI_PORT}.pid and try again."
      exit 0
    fi
  ...
  stop)
    if [ ! -f "$QPKG_DIR/sabnzbd-$WEBUI_PORT.pid" ]; then
      echo "SABnzbdPlus hasn't been enabled or started ..."
      # Clean up symlinks in event SABnzbdPlus was likely shutdown outside QPKG manager
